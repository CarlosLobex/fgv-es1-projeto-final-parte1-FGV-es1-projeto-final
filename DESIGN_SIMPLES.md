...
### 1.1 Rotas diretas e sem abstração desnecessária (rotas de perguntas em `server.js`)

O backend do ESM Forum define rotas RESTful de forma direta em `server.js`, sem camadas de abstração intermediárias (como repositórios ou services) que não seriam necessárias para um sistema de escopo didático.

**Exemplo típico da estrutura encontrada:**

```js
// server.js — rotas de perguntas
...
### 1.3 Banco de dados SQLite sem ORM (rotas de respostas em `server.js`)
